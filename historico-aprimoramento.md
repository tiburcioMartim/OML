...
|---|---|
| Push: pull-first + alvo homologação + listar projetos (2026-06-08) | ✅ **Promovido à matriz** (engine) nesta rodada, após revisão: `docs/04-protocolos/protocolo-push.md` (nova etapa "0. Sincronizar (pull)" + etapa 3 com branch-alvo homologação/fallback produção e lista de projetos; ações proibidas e checklist), `docs/01-comandos/push.md` (ações obrigatórias/proibidas + frase de encerramento). Origem: relatório de aprimoramento do `rhc-erp-laravel`. |
| **Cofre de Acessos do OML (`acessos.local.env`) — o "`.env` próprio" do agente** (2026-06-08) | ✅ **Aplicado na matriz** (engine): novo `docs/04-protocolos/protocolo-acessos-oml.md` + template `docs/05-templates/acessos.example.env`; seed-if-missing no `docs/01-comandos/migracao-ativar.md` (garante `.gitignore`, cria `acessos.example.env`/`acessos.local.env`, reporta blocos **mascarados**); `.gitignore` (`*.local.env`); `oml.manifesto.json` (bloco `acessos` + `local` + migração 1.4.0); `docs/03-guardioes/guardiao-segredos-credenciais.md` (regras duras do cofre); `docs/05-templates/perfil-projeto.template.md` (bloco do cofre); `README.md`/`CHANGELOG.md`; bump **v1.4.0**. |
...
| URLs de teste obrigatórias ao finalizar uma tela (2026-06-08) | ✅ **Aplicado na matriz E no projeto** (dev autorizou explicitamente): `docs/04-protocolos/protocolo-homologacao.md` reescrito com a **regra obrigatória** — ao finalizar uma página/handoff, encerrar a resposta com a(s) **URL(s) válida(s) de teste** montada(s) da rota real (`<base>/<uri>` da `rota_laravel`, confirmada no Ziggy/`routes`), só GET navegável; rota com `{id}` → exemplo ou tela-índice; modal/sub-fluxo → URL da tela-mãe + como acionar; nunca chutar a URL. + item no checklist da `docs/00-visao-geral/definicao-de-pronto.md`. Classificação: **Núcleo genérico / Homologação / UX**. Origem: pedido do dev no `rhc-erp-laravel`. |

> **Origem (pedido do dev 2026-06-08):** "sempre que finalizar uma página é obrigatório trazer as URLs válidas para o dev validar." O dev valida no navegador (Gate 2) e quer o link em mãos no fim da entrega, sem caçar a rota. Virou regra dura do Protocolo de Homologação + item da Definição de Pronto. Vira melhoria no GitHub: **sim** (aditivo, retrocompatível).
...
> **Origem (pedido do dev 2026-06-08):** "você vai passar a ter um meio que `.env` seu próprio da OML, ele servirá para configurar acessos — por exemplo, a gente precisa logar no ERP; então neste `.env` teriam os acessos, para você só resolver o problema". O OML investigava e documentava, mas **não tinha um lugar próprio para os acessos operacionais** — logar no ERP, banco, integrações — e dependia do dev executar cada passo. Resposta: **Cofre de Acessos** (`acessos.local.env`, gitignored) que o agente lê para **agir e resolver de ponta a ponta**, com `acessos.example.env` só-chaves versionado e regras duras de segredo (valores nunca impressos/commitados/na memória; 2FA não se burla). Distinto da Importação de `.env` do legado (que mapeia só **chaves**). Classificação: **Núcleo genérico / Segurança / Autonomia**. Prioridade: **Alta**. Vira melhoria no GitHub: **sim** (aditivo, retrocompatível).
...
| 2026-06-08 | O OML investigava e documentava, mas não tinha um lugar próprio para os **acessos operacionais** (logar no ERP legado/novo, banco, integrações) — dependia do dev executar cada login/consulta, então não conseguia "só resolver o problema" de ponta a ponta | Núcleo genérico / Segurança / Autonomia | Criar o **Cofre de Acessos do OML**: `acessos.local.env` (valores reais, **gitignored** via `*.local.env`, classificado `local` no manifesto) + `acessos.example.env` (só **chaves**, versionado). O agente lê o cofre para **autenticar e agir** (ex.: Playwright loga no ERP). Seed-if-missing no `/migracao-ativar` (garante `.gitignore`, cria os arquivos, reporta blocos **mascarados**). Regras duras: valores só no `.local.env`, nunca imprimir/ecoar/commitar/colar-no-chat/copiar-p/-memória; 2FA/captcha não se burla; acesso não dispensa gates. Distinto da Importação de `.env` do legado (que mapeia só chaves). | Alta |
