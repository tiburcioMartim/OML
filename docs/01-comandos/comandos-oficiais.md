...
7. Todos estes comandos estão registrados como **slash commands do Claude Code** em `.claude/commands/` (motor, versionado) — aparecem no autocomplete do `/` quando o diretório de trabalho é o clone do OML. Cada arquivo delega ao doc correspondente em `docs/01-comandos/`. Para tê-los **dentro do projeto novo** (onde se codifica), gere cópias equivalentes em `.claude/commands/` do projeto na configuração, ou instale-os em `~/.claude/commands/` (global).
