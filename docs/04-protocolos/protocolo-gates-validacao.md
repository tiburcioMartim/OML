...
| `/migracao-homologar [ID]` (15 — **Gate 2**) | veredito de **segurança verificado** no ledger `memoria/seguranca/{slug}.md` — **só em tela de alta criticidade** (dado de saúde, cálculo clínico/financeiro) | 👤 Humano 🔴 | **ABORTA:** "Segurança não verificada (10 eixos OWASP/ASVS/CWE; sem rebaixamento; L3 no dado de saúde). Rode `/migracao-mapear-seguranca [ID]`." Tela de baixa criticidade não bloqueia. |
